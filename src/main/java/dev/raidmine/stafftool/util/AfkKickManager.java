    ...
    public static void tick(MinecraftClient client) {
        ...
        if (!sentToHub && idleMillis >= thresholdMillis) {
            ...
            if (!command.isBlank()) {
                ...
                MutableText prefix = Text.literal("[RaidMine Tools]")
                        .setStyle(Style.EMPTY.withColor(RaidMineStaffMod.config().accentColor));
                Text body = Text.literal(" Вы были перемещены на сервер Hub, вы бездействовали больше 4 минут, поэтому были отправлены сюда.");
