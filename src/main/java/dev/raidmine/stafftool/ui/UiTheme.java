...
public final class UiTheme {
    ...
    /** High-quality procedural panel texture with smooth scattered glows. */
    public static void panelTexture(DrawContext context, int x, int y, int width, int height, int accentColor) {
        ...
        int major = Math.max(18, Math.round(min * 0.38F));
        int medium = Math.max(14, Math.round(min * 0.26F));
        int small = Math.max(10, Math.round(min * 0.18F));

        softGlow(context, x + Math.round(width * 0.22F), y + Math.round(height * 0.32F),
                major, Math.max(10, Math.round(major * 0.55F)), accentColor, Math.round(20F * opacity));
        softGlow(context, x + Math.round(width * 0.73F), y + Math.round(height * 0.64F),
                medium, Math.max(9, Math.round(medium * 0.60F)), accent2(), Math.round(16F * opacity));
        softGlow(context, x + Math.round(width * 0.58F), y + Math.round(height * 0.22F),
                small, Math.max(7, Math.round(small * 0.65F)), blend(accentColor, 0xFFFFFFFF, 0.18F), Math.round(9F * opacity));
        softGlow(context, x + Math.round(width * 0.30F), y + Math.round(height * 0.76F),
                small, Math.max(7, Math.round(small * 0.58F)), accent2(), Math.round(8F * opacity));
    }

    /** Smooth, layered glow with soft falloff and better anti-aliasing. */
    public static void softGlow(DrawContext context, int centerX, int centerY,
                                int radiusX, int radiusY, int color, int maxAlpha) {
        if (maxAlpha <= 0 || radiusX <= 0 || radiusY <= 0) return;
        int layers = 22;
        for (int i = layers; i >= 1; i--) {
            float t = i / (float) layers;
            float eased = 1F - (1F - t) * (1F - t);
            int rx = Math.max(1, Math.round(radiusX * eased));
            int ry = Math.max(1, Math.round(radiusY * eased));
            float falloff = 1F - t;
            int alpha = Math.max(1, Math.round(maxAlpha * falloff * falloff * falloff * 1.15F));
