...
public final class SettingsScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        float progress = Math.min(1F, (System.currentTimeMillis() - tabChangedAt) / 460F);
        ...
        if (tabChangedAt > 0L && progress < 1F && previousTab != tab) {
            ...
            int oldOffset = Math.round(viewport.w() * 0.18F * eased) * -direction;
            int newOffset = Math.round(viewport.w() * 0.22F * (1F - eased)) * direction;
            renderTab(context, previousTab, l, mouseX - oldOffset, mouseY, oldOffset,
                    1.00F - 0.02F * eased, false);
            renderTab(context, tab, l, mouseX - newOffset, mouseY, newOffset,
                    0.975F + 0.025F * eased, true);
        } else {
    ...
    private void renderHeader(DrawContext context, Layout l, int mouseX, int mouseY) {
        ...
        UiTheme.text(context, textRenderer, "RM Tools", l.x() + 80, l.y() + 17, 16F, UiTheme.TEXT, true);
        UiTheme.text(context, textRenderer, "Персонализация, модерация и контроль чата",
                l.x() + 80, l.y() + 40, 9.4F, UiTheme.MUTED, false);
    ...
    private void renderSliderWithInput(DrawContext context, String label, Rect track, float value, TextFieldWidget field) {
        ...
        UiTheme.roundedRect(context, track.x(), track.y(), track.w(), track.h(), 5, UiTheme.argb(190, 44, 47, 58));
        int fillW = Math.round(track.w() * Math.max(0F, Math.min(1F, value)));
        if (fillW > 0) {
            int left = UiTheme.blend(UiTheme.accent(), 0xFF000000, 0.22F);
            context.fillGradient(track.x(), track.y(), track.x() + fillW, track.y() + track.h(), left, UiTheme.accent());
            if (fillW > 10) {
                context.fillGradient(track.x() + fillW / 2, track.y(), track.x() + fillW, track.y() + track.h(), UiTheme.accent(), UiTheme.accent2());
            }
        }
        int knobX = Math.max(track.x() - 1, Math.min(track.x() + track.w() - 11, track.x() + fillW - 6));
        UiTheme.softGlow(context, knobX + 6, track.y() + track.h() / 2, 7, 7, UiTheme.accent(), 24);
        UiTheme.roundedRectExact(context, knobX, track.y() - 4, 12, 16, 6, UiTheme.TEXT);
    ...
    private void input(DrawContext context, TextFieldWidget field, String placeholder) {
        ...
        UiTheme.text(context, textRenderer, value.isBlank() ? placeholder : value,
                field.getX() + 9, field.getY() + 10, 9F, value.isBlank() ? UiTheme.FAINT : UiTheme.TEXT, false);
    ...
    private void toggle(DrawContext context, Rect rect, String label, boolean value,
        ...
        UiTheme.text(context, textRenderer, label, rect.x() + 34, rect.y() + (rect.h() - 9) / 2, 9F, UiTheme.TEXT, true);
    ...
    private void button(DrawContext context, Rect rect, String label, boolean hovered, boolean accent, UiIcon icon) {
        ...
        UiTheme.text(context, textRenderer, label, textX, rect.y() + 11, 8F, UiTheme.TEXT, true);
    }
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (System.currentTimeMillis() - tabChangedAt < 460L) return true;
