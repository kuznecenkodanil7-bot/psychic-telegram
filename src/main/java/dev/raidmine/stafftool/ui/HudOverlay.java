...
public final class HudOverlay {
    ...
    private static void renderBar(DrawContext context, Layout l, SessionStats stats,
        ...
        int bottomGlow = notice == null ? (stats.goalReached() ? UiTheme.SUCCESS : border) : noticeAccent;
        drawBottomGlow(context, l.x() + 2, l.y() + 2, l.width() - 4, l.height() - 4, bottomGlow,
                Math.round((notice == null ? (stats.goalReached() ? 38F : 26F) : 30F)));
    ...
    private static void drawBottomGlow(DrawContext context, int x, int y, int width, int height, int color, int alpha) {
        if (alpha <= 0 || width <= 12 || height <= 12) return;
        context.enableScissor(x, y, x + width, y + height);
        int centerX = x + width / 2;
        int centerY = y + height + Math.max(2, height / 8);
        UiTheme.softGlow(context, centerX, centerY,
                Math.max(14, width / 2 - 8), Math.max(8, height / 2), color, alpha);
    ...
    private static void renderStatsContent(DrawContext context, Layout l, SessionStats stats,
        ...
        UiTheme.softGlow(context, cursor + m.logoSize() / 2, logoY + m.logoSize() / 2 + Math.max(1, Math.round(m.logoSize() * 0.06F)),
                Math.max(20, Math.round(m.logoSize() * 0.98F)),
                Math.max(13, Math.round(m.logoSize() * 0.72F)),
                0xFFFF8A00, Math.round(88F * alpha / 255F));
        context.disableScissor();
        ...
        int brandY = yBase + (l.height() - brandHeight) / 2 + Math.max(0, Math.round(0.3F * m.scale()));
        ...
        int clockY = blockY + (m.blockHeight() - m.iconSize()) / 2 + Math.max(0, Math.round(0.15F * m.scale()));
    ...
    private static int counterPanel(DrawContext context, TextRenderer tr, int x, int yBase, Layout l, Metrics m,
        ...
        if (active) {
            ...
            UiTheme.softGlow(context, x + width / 2, y + m.blockHeight() - Math.max(3, m.blockHeight() / 7),
                    Math.max(12, width / 2), Math.max(7, m.blockHeight() / 2),
                    accent, Math.round(40F * effect));
        ...
        int iconY = y + Math.max(2, Math.round(3.4F * m.scale()));
